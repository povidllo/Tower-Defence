...
namespace TDEngine::Inner {
	...
	void RendererGame::renderUI(const std::shared_ptr<GameStatus> &gameStat,
        ...
        // --- 2. Right panel (abilities + upgrades) ---
        ...
        float currentY = sideY + UI_PADDING + SECTION_HEADER_HEIGHT; // после заголовка
        ...
        if (!upgradeOptions.empty()) {
            ...
            // Рисуем каждое улучшение
            for (const auto& opt : upgradeOptions) {
                float iconX = sideX + UI_PADDING + 10.0f;
                float iconY = currentY;
                float iconSize = 40.0f; // немного меньше, чем у способностей

                // Фон кнопки
            	// Вместо drawRoundedBox используем простой прямоугольник
                ...
                // Сохраняем область клика (только для иконки, но можно и на всю строку)
